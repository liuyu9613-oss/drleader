...
export default async function handler(req, res) {
  ...
  try {
    ...
    let raw = extractReply(data);
    if (!raw) {
      console.error('LLM empty response:', JSON.stringify(redactForLog(data)).slice(0, 1000));
      raw = 'AI 服务暂时未返回有效内容，请稍后再试。';
    }
    const reply = String(raw).replace(/<think[\s\S]*?<\/think>\s*/gi, '').trim() || String(raw);
